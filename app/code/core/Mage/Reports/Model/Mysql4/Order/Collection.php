...
//            ->group('e.entity_id');
    ...
    public function setDateRange($from, $to)
    {
        ...
        {
        	$order_ids = $this->getOrderIds($user_id);
        	$productIds = $this->getProductIds($user_id);
    ...
    public function joinCustomerName()
    {
        ...
        $this->getSelect()->from("", array('cus_name' => 'CONCAT(_table_firstname.value," ", _table_lastname.value)'));
        return $this;
    }
 ...
 /**
     * ***********************************************************
     * Get All Product Ids paticular Seller id
     * @param unknown_type $seller_id
     * @return unknown
     */
    public function getProductIds($seller_id = 0)
    {
    	$productCollection = Mage::getModel('catalog/product')->getCollection();
    	$productCollection->addFieldToFilter('seller_id',$seller_id);
    	$productIds = array();
    ...
    /**
     * *****************************************************
     * Get All Order Ids paticular Seller id
     * @param unknown_type $seller_id
     * @return unknown
     */
    public function getOrderIds($seller_id = 0)
    {
    	$productIds = $this->getProductIds($seller_id);
    	$orders = Mage::getSingleton('sales/order_item')->getCollection()
					->addFieldToFilter('product_id',array('in'=>$productIds));
		$order_id1 = array();
		foreach ($orders as $order):
			$order_id1[] = $order['order_id'];
		endforeach;
		
		$orders1 = Mage::getResourceModel('sales/order_collection')
		            ->addAttributeToSelect('entity_id')
		           ->addAttributeToFilter('state', array('neq'=> Mage_Sales_Model_Order::STATE_CANCELED))
		            ->addAttributeToFilter('entity_id',array('in'=>$order_id1))
...
/**
     ...
     * @return Mage_Reports_Model_Mysql4_Order_Collection
     */
    public function addSellerSumAvgTotals($storeId = 0)
    {
        if ($storeId == 0) {
            /**
             * Join store_to_base_rate attribute
             */
        	$user_id = Mage::getSingleton('admin/session')->getUser()->getUserId();
        	$order_ids1 = $this->getOrderIds($user_id);
             ...
             */
            //$expr = "(e.base_subtotal-IFNULL(e.base_subtotal_refunded,0)-IFNULL(e.base_subtotal_canceled,0))*_b2gr_{$baseToGlobalRateTableName}.{$baseToGlobalRateTableName}";
            $flatOrderTable = 'usez_sales_flat_order_item';
            if(count($order_ids1)> 0)
            {
                $this->getSelect()
        		->joinInner(array('usez_sales_flat_order_item' =>$flatOrderTable),
        		"{$flatOrderTable}.order_id = e.entity_id")
        		->where("e.entity_id IN ".$order_ids);
            }
            else 
            {
            	$this->getSelect()
            ...
            $expr = "usez_sales_flat_order_item.base_row_total";
