...
class Mage_Checkout_Model_Type_Onepage
{
    ...
    public function saveBilling($data, $customerAddressId)
    {
        ...
        if (!$this->getQuote()->getCustomerId() && Mage_Sales_Model_Quote::CHECKOUT_METHOD_REGISTER == $this->getQuote()->getCheckoutMethod()) {
            ...
            if ($this->_customerNricExists($address->getNric(), Mage::app()->getWebsite()->getId())) {
                return array('error' => 1,
                    'message' => Mage::helper('checkout')->__('There is already a customer registered using this NRIC')
                );
            }
        }
    ...
    public function saveOrder()
    {
        ...
        case Mage_Sales_Model_Quote::CHECKOUT_METHOD_REGISTER:
			...
			$customer->setData('parent_id',$billing->getParentId());
			$customer->setData('nric',$billing->getNric());
        ...
        if ($this->getQuote()->getCheckoutMethod() == Mage_Sales_Model_Quote::CHECKOUT_METHOD_REGISTER) {
            if ($this->_customerNricExists($customer->getNric(), Mage::app()->getWebsite()->getId())) {
                Mage::throwException(Mage::helper('checkout')->__('There is already a customer registered using this NRIC'));
            }
        }
        $order->place();
    ...
    /**
     * Check if customer nric exists
     *
     * @param string $nric
     * @param int $websiteId
     * @return false|Mage_Customer_Model_Customer
     */
    protected function _customerNricExists($nric, $websiteId = null)
    {
