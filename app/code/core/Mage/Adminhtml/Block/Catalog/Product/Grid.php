...
class Mage_Adminhtml_Block_Catalog_Product_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareCollection()
    {
        ...
        $affiliate_id = $this->getRequest()->getParam('affiliate');
        $store = $this->_getStore();
        $collection = Mage::getModel('catalog/product')->getCollection();
        if($user_role == 'Affiliate')
        {
        	$collection = $collection->addFieldToFilter('seller_id',$user_id);
        }
        else 
        {
        	if($affiliate_id != null)
        	{
        		$admin_model = Mage::getModel('admin/user')->getCollection();
        		$admin_model->getSelect()->where('customer_id = ?',$affiliate_id);
        		$admin_data = $admin_model->getData();
        		if(isset($admin_data[0]['user_id']))
        			$collection = $collection->addFieldToFilter('seller_id',$admin_data[0]['user_id']);
        	}
        }
    ...
    protected function _prepareColumns()
    {
        ...
        $this->addColumn('qty',
         ...
         $this->addColumn('affiliate_id',
            array(
                'header'=> Mage::helper('catalog')->__('Affiliate ID'),
                'width' => '100px',
                'index' => 'affiliate_id',
        ));
