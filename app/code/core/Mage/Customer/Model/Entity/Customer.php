...
class Mage_Customer_Model_Entity_Customer extends Mage_Eav_Model_Entity_Abstract
{
    ...
    protected function _beforeSave(Varien_Object $customer)
    {
		...
		$select1 = $this->_getReadAdapter()->select()
            ->from($this->getEntityTable(), array($this->getEntityIdField()))
            ->where('nric=?', $customer->getNric());
        if ($customer->getSharingConfig()->isWebsiteScope()) {
            $select1->where('website_id=?', (int) $customer->getWebsiteId());
        }
        if ($customer->getId()) {
            $select1->where('entity_id !=?', $customer->getId());
        }
		else{
        if ($this->_getWriteAdapter()->fetchOne($select1)) {
            Mage::throwException(Mage::helper('customer')->__('Customer NRIC/Passport already exists'));
        }
        }
