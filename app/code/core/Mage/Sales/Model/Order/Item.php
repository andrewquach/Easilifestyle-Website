...
class Mage_Sales_Model_Order_Item extends Mage_Core_Model_Abstract
{
    ...
    protected function _beforeSave()
    {
        ...
        $this->setBaseBv($this->_calcRowBaseBv());
        $this->setBaseGl($this->_calcRowBasegl());
        $this->setAffiliateAmount($this->_CalculateAffiliateAmount());
        $this->setAffiliateId($this->_getProductAffiliateId2());
        return $this;
    }
     ...
     * USEZ function
     */
 /*USEZ calculate BV va GL cho order Items*/
    protected function _calcRowBaseBv()
    {
    	$product_id = $this->getProductId();
    	$product = Mage::getModel('catalog/product')->load($product_id);
    	$qty_order = $this->getQtyOrdered();
    	$bv = $product->getBv();
    	return $bv*$qty_order;
    }
	protected function _calcRowBaseGl()
    {
    	$product_id = $this->getProductId();
    	$product = Mage::getModel('catalog/product')->load($product_id);
    	$qty_order = $this->getQtyOrdered();
    ...
    protected function _getProductAffiliateId()
    {
    	$product_id = $this->getProductId();
    	$product = Mage::getModel('catalog/product')->load($product_id);
    	...
    	if($affiliate_id>0)
    	{
    		$admin_model = Mage::getModel('admin/user')->load($affiliate_id);
    		$customer_id = $admin_model->getCustomerId();
    		return $customer_id;
    	}
    	return $affiliate_id;
    }
	protected function _getProductAffiliateId2()
    {
    	$product_id = $this->getProductId();
    	$product = Mage::getModel('catalog/product')->load($product_id);
    	$affiliate_id = $product->getAffiliateId();    	
    	return $affiliate_id;
    }
    protected  function _CalculateAffiliateAmount()
    {
    	...
    	if($affiliate_id > 0)
    	{
    		$percent = $product->getAdminCommission();
    		$affiliate_amount = $this->getPrice() * $this->getQtyShipped() * (100 - $percent) /100 ;
