...
class Mage_Sales_Model_Order extends Mage_Sales_Model_Abstract
{
    ...
    public function canCreditmemo()
    {
         ...
         */
        if (abs($this->getTotalPaid()-$this->getTotalRefunded())<0) {
            return false;
        }
...
/*********************** STATUSES ***************************/
    ...
    protected function _afterSave()
    {
    	...
    	{
    		$this->_calculateAffiliateAmount();
			$this->__SaveBV();
			$this->__SaveGL();
    	}
    	return $this;
    ...
    protected function __SaveBV()
    {
    	$increment = $this->getIncrementId();
    	$order_ids = explode('-',$increment);
    	$order_id = $order_ids[0];
    	$groupbv_collection = Mage::getModel('ambassador/groupbv')->getCollection();
    	$groupbv_collection->getSelect()->where('order_id =?',$order_id);
    	$groupbv_data = $groupbv_collection->getData();
    	if(count($groupbv_data) > 0)
    	{
    		$groupbv_id = $groupbv_data[0]['id'];
    		$groupbv = Mage::getModel('ambassador/groupbv')->load($groupbv_id);
    	}
    	else 
    	{
    		$groupbv = Mage::getModel('ambassador/groupbv')->setId(null);
    	}
    	$bv = $this->__calculateBV($this->getId());
    	$customer = $this->__getCustomer();
    	$groupbv->setCustomerId($this->getCustomerId());
    	...
    	$groupbv->setCreateDate(date('Y-m-d'));
    ...
    protected function __calculateBV($order_id)
    {
    	...
    	$order_items->getSelect()->where('order_id = ?',$order_id)->where('product_type != \'configurable\'');
    	$order_item_data = $order_items->getData();
	...
	protected function __SaveGL()
    {
    	$glexp = Mage::getStoreConfig('admin/usezcommon/glexpiration');
    	$increment = $this->getIncrementId();
    	$order_ids = explode('-',$increment);
    	$order_id = $order_ids[0];
    	$groupgl_collection = Mage::getModel('ambassador/groupgl')->getCollection();
    	$groupgl_collection->getSelect()->where('order_id =?',$order_id);
    	$groupgl_data = $groupgl_collection->getData();
    	if(count($groupgl_data) > 0)
    	{
    		$groupgl_id = $groupgl_data[0]['id'];
    		$groupgl = Mage::getModel('ambassador/groupgl')->load($groupgl_id);
    	}
    	else 
    	{
    		$groupgl = Mage::getModel('ambassador/groupgl')->setId(null);
    	}
    	$gl = $this->__calculateGL($this->getId());
    	$customer = $this->__getCustomer();
    	$groupgl->setCustomerId($this->getCustomerId());
    	$groupgl->setCustomerFirstname($customer->getFirstname());
    	$groupgl->setCustomerLastname($customer->getLastname());
    	$groupgl->setGl($gl);
    	$groupgl->setGlParentId($customer->getParentId());
    	$groupgl->setOrderId($order_id);
    	$groupgl->setCreateDate(date('Y-m-d'));
    	$groupgl->setGlExpirationDate(date('Y-m-d',strtotime('+ '.$glexp.' years - 1 days')));
    	$groupgl->setStatus('unused');
    	$groupgl->save();
    	
    	
    }
    protected function __calculateGL($order_id)
    {
    	$groupgl = 0;
    	$order_items = Mage::getModel('sales/order_item')->getCollection();
    	$order_items->getSelect()->where('order_id = ?',$order_id)->where('product_type != \'configurable\'');
    ...
    protected function __getCustomer()
    {
    	$customer = Mage::getModel('customer/customer')->load($this->getCustomerId());
    	return $customer;
    }
    protected function _calculateAffiliateAmount()
    {
		$increment = $this->getIncrementId();
    	$order_ids = explode('-',$increment);
    	$order_id = $order_ids[0];
    	...
    	$order_items = Mage::getModel('sales/order_item')->getCollection();
    	$order_items->getSelect()->where('order_id = ?',$this->getId())->where('product_type != \'configurable\'');
    	$order_item_data = $order_items->getData();
    	foreach ($order_item_data as $k=>$v){
			$affiliateid = $v['affiliate_id'];
			...
			{
				$affiliatebv = Mage::getModel('ambassador/affiliatebv')->setId(null);
				$affiliatebv->setCustomerId($parents['entity_id']);
				$customer = Mage::getModel('customer/customer')->load($parents['entity_id']);
				$affiliatebv->setCustomerFirstname($customer->getFirstname());
				$affiliatebv->setCustomerLastname($customer->getLastname());		
				$affiliatebv->setOrderId($order_id);
				$affiliatebv->setCreateDate(date('Y-m-d'));					
				$affiliatebv->setBv($v['base_bv']);
				$affiliatebv->save();
			}			
    	}    	
    }
