...
//        }
	...
	public function addAffiliateNamesToResult()
    {
       	$affiliate_ids = array();
       	$affiliate_collection = Mage::getModel('admin/user')->getCollection()->getData();
       	foreach ($affiliate_collection as $k=>$v):
       		$affiliate_ids[$v['user_id']] = $v['firstname'].' '.$v['lastname'];
       	endforeach;
        foreach ($this as $product) {
            if (isset($affiliate_ids[$product->getSellerId()])) {
                $product->setData('affiliate',$affiliate_ids[$product->getSellerId()]);
            }
            else 
            {
            	 $product->setData('affiliate','Administrator');	
            }
        }
        return $this;
    }
